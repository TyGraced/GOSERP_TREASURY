...
namespace GODP.APIsContinuation.Handlers.Supplier
{
    public class DeleteSupplierCommandHandler : IRequestHandler<DeleteSupplierCommand, DeleteRespObj>
    {
        ...
        public DeleteSupplierCommandHandler(ISupplierRepository supplierRepository, DataContext dataContext, ILoggerService loggerService)
        {
                ...
                using (var _transaction = await _dataContext.Database.BeginTransactionAsync())
                {
                    try
                    {
                        if (request.req.Count() > 0)
                        {
                            ...
                            {
                                var itemToDelete = await _supRepo.GetSupplierAsync(itemId.TargetId);
                                await _supRepo.DeleteSupplierAsync(itemToDelete);
                            }
                        }
                    ...
                    catch (SqlException ex)
                    {
                        ...
                        _logger.Error($"ErrorID : DeleteSupplierCommandHandler{errorCode} Ex : {ex?.Message ?? ex?.InnerException?.Message} ErrorStack : {ex?.StackTrace}");
                        return new DeleteRespObj
                        {
                            ...
                            Status = new APIResponseStatus
                            {
                                Message = new APIResponseMessage
                                {
                                    ...
                                    MessageId = errorCode,
                                    TechnicalMessage = $"ErrorID : DeleteSupplierCommandHandler{errorCode} Ex : {ex?.Message ?? ex?.InnerException?.Message} ErrorStack : {ex?.StackTrace}"
                                }
                            }
                        };
                        #endregion
