            ...
            try
            {
                using (var _transaction = await _dataContext.Database.BeginTransactionAsync())
                {
                    ...
                    catch (SqlException ex)
                    {
                        ...
                        _logger.Error($"ErrorID : DeleteSupplierTopClientCommandHandler{errorCode} Ex : {ex?.Message ?? ex?.InnerException?.Message} ErrorStack : {ex?.StackTrace}");
                        return new DeleteRespObj
                        {
                            ...
                            Status = new APIResponseStatus
                            {
                                Message = new APIResponseMessage
                                {
                                    ...
                                    MessageId = errorCode,
                                    TechnicalMessage = $"ErrorID : DeleteSupplierTopClientCommandHandler{errorCode} Ex : {ex?.Message ?? ex?.InnerException?.Message} ErrorStack : {ex?.StackTrace}"
                                }
                            }
                        };
                        #endregion
